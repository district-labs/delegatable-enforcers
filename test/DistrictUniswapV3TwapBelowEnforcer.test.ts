...
describe('DistrictUniswapV3TwapBelowEnforcer', () => {
  ...
  let DistrictUniswapV3TwapBelowEnforcerFactory: ContractFactory;
  ...
  before(async () => {
    ...
    DistrictUniswapV3TwapBelowEnforcerFactory = await ethers.getContractFactory(
      'DistrictUniswapV3TwapBelowEnforcer',
    );
  ...
  beforeEach(async () => {
    ...
    DistrictUniswapV3TwapAboveEnforcer = await DistrictUniswapV3TwapBelowEnforcerFactory.connect(
      wallet0,
  ...
  it('should SUCCEED to INVOKE method if tickThreshold below arithmeticMeanTick', async () => {
  ...
  it('should FAIL to INVOKE method if tickThreshold gte arithmeticMeanTick', async () => {
    const PK = wallet0._signingKey().privateKey.substring(2);
    expect(await ERC20Delegatable.balanceOf(wallet0.address)).to.eq(ethers.utils.parseEther('1'));
    ...
    ).to.be.revertedWith('DistrictUniswapV3TwapBelowEnforcer:tickThreshold >= uniswapTwapTick');
  });
